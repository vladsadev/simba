    ...
    <form wire:submit.prevent="submit">
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                    ...
                    <div class="px-6 py-4 bg-gray-50">
                        <div class="flex justify-between text-sm text-gray-600 mb-2">
                            ...
                            <span
                                class="font-semibold">{{ count($checkedItems) }}/{{ $this->total_items }} elementos revisados</span>
                        </div>
            ...
            {{ $this->progress < 40 ? 'bg-red-500' : ($this->progress < 80 ? 'bg-yellow-500' : 'bg-green-500') }}"
                    ...
                    <div class="p-6 space-y-3">
                        @foreach($inspectionItems as $key => $item)
                            @if($key === 'br')
                                {{-- Mejorar el estilo del subtítulo --}}
                                <div class="section-divider mt-6 mb-4">
                                    <h3 class="text-lg font-semibold text-gray-800 border-b-2 border-blue-200 pb-2">
                                        {{ $sectionTitles[$key] ?? 'Nueva Sección' }}
                                    </h3>
                                </div>
                            @else
                                <div class="inspection-item flex items-center justify-between py-3 px-4 rounded-lg border-2
                transition-all duration-300
                {{ in_array($key, $checkedItems) ? 'bg-green-50 border-green-400' :
                   (isset($reportedIssues[$key]) ? 'bg-red-50 border-red-400' : 'bg-white border-gray-200 hover:border-blue-300') }}">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <input
                                            ...
                                            {{ in_array($key, $checkedItems) ? 'checked' : '' }}
                                            {{ isset($reportedIssues[$key]) ? 'disabled' : '' }}
                               ...
                               transition-colors duration-200">
                                  ...
                                  {{ in_array($key, $checkedItems) ? 'line-through text-green-600' : '' }}
                                  {{ isset($reportedIssues[$key]) ? 'text-red-600' : '' }}">
                                            {{ $item }}
                                        </label>
                                    </div>
                            ...
                            {{ ucfirst($reportedIssues[$key]['severidad']) }}
                        </span>
                                        @endif
                               ...
                               {{ isset($reportedIssues[$key]) ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-yellow-100 text-yellow-600 hover:bg-yellow-200' }}
                               transition-colors duration-200"
            ...
            <div class="lg:col-span-2 space-y-4">
                            ...
                            @foreach($reportedIssues as $key => $issue)
                                <div class="text-sm border-l-4 border-red-500 pl-3 py-2">
                                    ...
                                    <div class="text-gray-600">
                                        Severidad: <span
                                            class="font-medium text-red-600">{{ ucfirst($issue['severidad']) }}</span>
                                    </div>
                ...
                <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                    <div class="bg-blue-main px-4 py-3">
        ...
        <div class="mt-6 flex justify-between items-center">
            ...
            <div class="space-x-3">
                ...
                <button type="submit"
                        ...
                        class="px-6 py-2 bg-blue-lighter hover:bg-blue-700 text-white rounded-md font-medium transition-colors">
